...
begin
  ...
  begin
    ...
    if FileExists(QQWryFile) then begin
      ...
      MemoPCInfo.Lines.Add('-----QQWry.dat数据库-------');
...
begin
  ...
  count:=size div sizeof(win32_find_data);
...
var
  remoteDir,LocalDir,LocalFile:string;
...
var
  ro:stRegOpInfo;
  Key,RootKey,NewKey,Val,data:string;
  p:pansiChar;
  i:integer;
begin
  if not CurConActive() then exit;
